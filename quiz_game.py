...
# 퀴즈 목록과 최고 점수를 저장할 파일입니다.
STATE_FILE = Path("state.json")


def create_default_quizzes():
    """저장 파일이 없거나 손상되었을 때 사용할 기본 퀴즈 목록을 만듭니다."""
    return [
        Quiz(
        ...
        Quiz(
            "Python에서 여러 값을 순서대로 저장할 때 주로 사용하는 자료형은 무엇인가요?",
            ["bool", "int", "list", "str"],
            3,
        ),
        Quiz(
            "조건에 따라 다른 코드를 실행할 때 사용하는 키워드는 무엇인가요?",
            ["if", "for", "def", "class"],
            1,
        ),
        Quiz(
        ...
        Quiz(
            "객체를 만들기 위한 설계도 역할을 하는 문법은 무엇인가요?",
            ["클래스", "딕셔너리", "문자열", "연산자"],
            1,
        ),
    ]
...
class QuizGame:
    """퀴즈 목록, 점수, 메뉴 흐름, 파일 저장과 불러오기를 관리합니다."""

    def __init__(self):
        # 먼저 기본 퀴즈를 준비하고, 저장 파일이 있으면 아래 load()에서 덮어씁니다.
    ...
    def get_number_input(self, prompt, minimum, maximum):
        """지정된 범위 안의 숫자를 입력받을 때까지 반복합니다."""
        while True:
            try:
                # 앞뒤 공백을 제거해서 ' 1 ' 같은 입력도 정상 처리합니다.
                raw_value = input(prompt).strip()
            except (KeyboardInterrupt, EOFError):
                # Ctrl+C 또는 입력 종료가 발생해도 비정상 종료되지 않도록 None을 반환합니다.
                print()
    ...
    def run(self):
        """메뉴를 반복해서 보여 주고 선택한 기능을 실행합니다."""
        while True:
            self.display_menu()
            menu = self.get_number_input("선택: ", 1, 5)

            # 입력이 중단되면 가능한 범위에서 저장하고 종료합니다.
            if menu is None:
                self.save()
                break
    ...
    def save(self):
        ...
        try:
            # ensure_ascii=False로 설정해 한글이 깨지지 않고 그대로 저장되게 합니다.
    ...
    def load(self):
        """state.json에서 저장된 퀴즈와 최고 점수를 불러옵니다."""
        ...
        try:
            ...
            # 저장된 딕셔너리 목록을 Quiz 객체 목록으로 복원합니다.
            ...
            print(
                f"저장된 데이터를 불러왔습니다. "
                f"퀴즈 {len(self.quizzes)}개, 최고 점수 {score_text}점"
            )
        except (OSError, json.JSONDecodeError, KeyError, TypeError, ValueError):
            # 파일이 없거나 손상되어도 프로그램을 계속 사용할 수 있도록 기본값으로 복구합니다.
    ...
    def get_text_input(self, prompt):
        """빈 문자열이 아닌 텍스트를 입력받을 때까지 반복합니다."""
        while True:
            try:
                value = input(prompt).strip()
            except (KeyboardInterrupt, EOFError):
                print()
                print("입력이 중단되었습니다. 가능한 범위에서 저장하고 종료합니다.")
                return None
